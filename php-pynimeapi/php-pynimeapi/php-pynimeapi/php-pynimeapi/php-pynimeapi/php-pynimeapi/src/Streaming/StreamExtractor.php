...
class StreamExtractor {
    ...
    public function __construct() { $this->client = new Client(['headers' => ['User-Agent' => 'Mozilla/5.0']]); }
    private function getEmbedUrl(string $episode_url): string {
        ...
        $document = new Document($html); $iframe = $document->first('iframe'); return "https:".$iframe->getAttribute('src');
    }
    ...
    public function extract(string $url): ?string {
        try {
            $url = $this->getEmbedUrl($url); $parsed_url = parse_url($url); parse_str($parsed_url['query'], $query_params);
            $content_id = $query_params['id']; $next_host = "{$parsed_url['scheme']}://{$parsed_url['host']}/";
            $streaming_page = $this->client->get($url, ['headers' => ['Referer' => $next_host]])->getBody()->getContents();
            preg_match_all('/(?:container|videocontent)-(\d+)/', $streaming_page, $key_matches);
            [$encryption_key, $iv, $decryption_key] = $key_matches[1];
            preg_match('/data-value="(.+?)"/', $streaming_page, $data_matches); $encrypted_data = $data_matches[1];
            ...
            $ajax_response = $this->client->get($next_host . "encrypt-ajax.php?" . $component, ['headers' => ['x-requested-with' => 'XMLHttpRequest', 'Referer' => $url]]);
