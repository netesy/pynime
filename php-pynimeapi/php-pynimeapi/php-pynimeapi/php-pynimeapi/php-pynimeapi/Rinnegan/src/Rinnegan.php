...
class Rinnegan {
    ...
    public function search_anime(string $anime_title): ?array {
        $anime_result = [];
        try {
            $response = $this->client->get("https://ajax.gogo-load.com/site/loadAjaxSearch", ['query' => ['keyword' => $anime_title]]);
            $data = json_decode($response->getBody()->getContents(), true); if(empty($data['content'])) return null;
            $document = new Document($data['content']); $items = $document->find('a.ss-title');
            foreach ($items as $item) {
                $title = $item->text(); $category_url = $this->baseURL . $item->getAttribute('href');
    ...
    public function get_anime_details(string $anime_category_url): ?AnimeDetailsObj {
        try {
            $response = $this->client->get($anime_category_url); $html = $response->getBody()->getContents();
            $document = new Document($html); $info_body = $document->first('.anime_info_body_bg');
            $title = $info_body->first('h1')->text(); $image_url = $info_body->first('img')->getAttribute('src');
    ...
    public function get_episode_urls(string $anime_category_url): ?array {
        try {
            $response = $this->client->get($anime_category_url); $html = $response->getBody()->getContents();
            preg_match('/<input.+?value="(\d+)" id="movie_id"/', $html, $matches); $anime_id = $matches[1] ?? null; if (!$anime_id) return null;
            $response = $this->client->get("https://ajax.gogo-load.com/ajax/load-list-episode", ['query' => ['ep_start' => 0, 'ep_end' => 9999, 'id' => $anime_id,]]);
            $html = $response->getBody()->getContents(); $document = new Document($html);
            $eps_list = []; $links = $document->find('a');
            foreach ($links as $link) { $eps_list[] = $this->baseURL . trim($link->getAttribute('href')); }
    ...
    public function grab_stream(string $anime_title, int $episode, int $resolution = 1080): ?string {
        ...
        if (empty($episode_urls) || !isset($episode_urls[$episode - 1])) { return null; }
        $stream_urls = $this->get_stream_urls($episode_urls[$episode - 1]); if (empty($stream_urls)) return null;
        if (isset($stream_urls[$resolution])) { return $stream_urls[$resolution]; }
        else { return reset($stream_urls); }
    }
    public function get_schedule(): void {
        $schedule = new Schedule(); $unix_time = time(); $schedule_data = $schedule->fetchSchedule($unix_time);
        $template = [];
        foreach ($schedule_data as $airing) {
            $datetime = date('M. d, l', $airing['airingAt']); $time = date('H:i:s', $airing['airingAt']);
            $template[$datetime][$time][] = ['name' => $airing['media']['title']['userPreferred'], 'episode' => $airing['episode']];
        }
        foreach ($template as $date_format => $child_component) {
            ...
            foreach ($child_component as $time_format => $anime_component) {
                echo "\t{$time_format} - "; $animes = [];
                foreach ($anime_component as $anime) { $animes[] = "{$anime['name']} [{$anime['episode']}]"; }
                echo implode("\n\t\t - ", $animes) . "\n";
            }
        }
    }
    public function get_recent_release(int $page = 1): ?array {
        try {
            $response = $this->client->get("https://ajax.gogo-load.com/ajax/page-recent-release.html", ['query' => ['page' => $page]]);
